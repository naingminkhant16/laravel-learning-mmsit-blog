...
<x-card>
    ...
    <x-input name="title" label='Post Title' :defaultValue="$post->title" form='postUpdateForm' />
    <div class="mb-3">
        <label class="form-label" for="category">Select Category</label>
        <select type="text" class="form-select @error('category') is-invalid @enderror" name="category" id="category"
            form="postUpdateForm">
            @foreach($categories as $category)
            <option value="{{ $category->id }}" {{ $category->id == old('category',$post->category) ?
                'selected':''
                }}>
                {{ $category->title }}
            </option>
    ...
    <div class="mb-3">
        ...
        <div class="">
            ...
            <input type="file"
                ...
                name="photos[]" id="photos" multiple form="postUpdateForm">
            @error("photos")
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
            @error("photos.*")
    ...
    <div class="mb-3">
        ...
        <textarea type="text" class="form-control @error('description') is-invalid @enderror" rows="10"
            name="description" id="description"
            form="postUpdateForm">{{ old('description',$post->description) }}</textarea>
        @error("description")
        <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
